...
$(function(){
	...
	var PageClick = function(pageclickednumber,sumcount){
		...
		//提供的外方接口用于重写
		var url=Global+"/pivi_listAjax.action";
		var data={};
		data["pageindex"]=pageclickednumber;
		data["pageReows"]=DATA_ROWS;
		if(searchTest&&searchTest!=''){data["searchTest"]=searchTest;}
		...
		$.post(url, data, function(result){
		 ...
		 if(json.success){
			 ...
			 for(var x=0;x<json.obj.length;x++){
				 json.obj[x].msg=submax10(json.obj[x].url);
			 }
	...
	$("#searchbutton").click(function(){
		var val= $("#search").val();
		if(val&&val!=''){
			var url=Global+"/pivi_privileges.action?searchText="+val;
			toMain(url);
		}else{
  ...
  function closecanvas(){
	...
	$("#addPivi").click(function(){
		 toMain(Global+"/pivi_addPrivilege.action");
	});
	function initEditfun(){
		$("a[edit-emp]").click(function(){
			var id= $(this).attr("edit-emp");
			toMain(Global+"/empl_editPrivilege.action?piviId="+id);
		});
	}
