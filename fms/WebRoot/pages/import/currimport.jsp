...
<script type="text/javascript" src="<%=path%>/js/utils/jquery.tmpl.min.js"> </script>
  ...
  <body>
    <div class="modal-footer" style="text-align: left; padding-bottom: 0px;">
		<form id="uploadform" action="currencies_importData.action" target="excelupload" method="post" enctype="multipart/form-data" style="height:25px; width:160px; float: left;" >
	...
	$("#uploadbutton").click(function(){
	 ...
	 $("#mysaveData").click(function(){
		...
		paremt["sendStr"]=JSON.stringify(resultdata);
	      var url = Global+"/currencies_saveExcelData.action";
	      $.post(url,paremt,function(data){
		    	...
		    	$("#waitdiv").hide();
		    	alert(result.msg);
		    	var returnUrl = Global +"/currencies_findAllCurrencies.action";
		    	toMain(returnUrl);
		     });
	    ...
	    $("#download").click(function(){
	    	window.location.href=Global+"/fileDownload.action?fileFlag=currTemp";
	    });
	});
	...
	if(excelupload.attachEvent){ // IE  
    	...
    	$("#tbodycurr tr").remove();
	         ...
	         for(var x=0; x<mylist.length; x++){
	          ...
	          $("#SXrow").tmpl(json.obj).appendTo("#tbodycurr");  
	          }else{
	    	       $("#waitdiv").hide();
		...
		function sendfile(){
			...
			var url =Global+"/currencies_importData.action";
			toMain(url);
		}
		
		function clearErrorData(){
			//alert(resultdata);
			var url =Global+"/currencies_clearErrorData.action";
			var paremt={};
			paremt["sendStr"]=JSON.stringify(resultdata);
			$.post(url,paremt,function(data){
				var result=jQuery.parseJSON(data);
		    	...
		    	$("#tbodycurr tr").remove();
		    		//--------------
		    		var list = result.obj;
		    		$("#SXrow").tmpl(list).appendTo("#tbodycurr"); 
		    		//--------------
		    	}
			});
		}
