...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/utils/jquery.tmpl.min.js"> </script>
   ...
   var resultdata="";
	$(function(){
		var actionUrl = "${pageContext.request.contextPath}/outstorage_importData.action";
	...
	$("#uploadbutton").click(function(){
	 ...
	 $("#mysaveData").click(function(){
		...
		paremt["sendStr"]=JSON.stringify(resultdata);
	      var url = Global+"/outstorage_saveData.action";
	      $.post(url,paremt,function(data){
		    	...
		    	$("#waitdiv").hide();
		    	alert(result.msg);
		    	var returnUrl = Global +"/outstorage_findAllOutStorage.action";
		    	toMain(returnUrl);
		     });
	    ...
	    $("#download").click(function(){
	    	window.location.href=Global+"/fileDownload.action?fileFlag=outstorageTemp";
	    });
	});
	...
	if(excelupload.attachEvent){ // IE  
		...
		if(html&&html!=""){
			  ...
			  if(json.success){
		         ...
		         for(var x=0; x<mylist.length; x++){
		          ...
		          $("#SXrow").tmpl(json.obj).appendTo("#tbodystorage");  
		          }else{
		    	       $("#waitdiv").hide();
		...
		function clearErrorData(){
			//alert(resultdata);
			var url =Global+"/outstorage_clearErrorData.action";
			var paremt={};
			paremt["sendStr"]=JSON.stringify(resultdata);
			$.post(url,paremt,function(data){
				var result=jQuery.parseJSON(data);
		    	...
		    	$("#tbodystorage tr").remove();
		    		//--------------
		    		var list = result.obj;
		    		$("#SXrow").tmpl(list).appendTo("#tbodystorage"); 
		    		//--------------
		    	}
			});
		}
  ...
  <script id="SXrow" type="text/x-jquery-tmpl">
            <tr  {{if erroris}} style="color: red;" {{/if}}>
				...
				<td class="center">{{= expFlag}}　</td>
				<td class="center">{{= outStorageNo}}　</td>
				<td class="center">{{= orderNo}}　</td>
				<td class="center">{{= purchaseNo}}　</td>
				<td class="center">{{= hsCode}}　</td>
				<td class="center">{{= expQty}}　</td>
