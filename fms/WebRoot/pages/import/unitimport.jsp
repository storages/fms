...
<script type="text/javascript" src="<%=path%>/js/utils/jquery.tmpl.min.js"> </script>
  ...
  <body>
    <div class="modal-footer" style="text-align: left; padding-bottom: 0px;">
		<form id="uploadform" action="unit_importData.action" target="excelupload" method="post" enctype="multipart/form-data" style="height:25px; width:160px; float: left;" >
  ...
  <script type="text/javascript">
	...
	$("#uploadbutton").click(function(){
	 ...
	 $("#mysaveData").click(function(){
		...
		paremt["sendStr"]=JSON.stringify(resultdata);
	      var url = Global+"/unit_saveExcelData.action";
	      $.post(url,paremt,function(data){
		    	...
		    	$("#waitdiv").hide();
		    	alert(result.msg);
		    	var returnUrl = Global +"/unit_findAllUnit.action";
		    	toMain(returnUrl);
		     });
	    ...
	    $("#download").click(function(){
	    	window.location.href=Global+"/fileDownload.action?fileFlag=unitTemp";
	    });
	});
	...
	if(excelupload.attachEvent){ // IE  
		...
		if(html&&html!=""){
			  ...
			  if(json.success){
		         ...
		         for(var x=0; x<mylist.length; x++){
		          ...
		          $("#SXrow").tmpl(json.obj).appendTo("#tbodyunit");  
		          }else{
		    	       $("#waitdiv").hide();
		...
		function sendfile(){
			...
			var url =Global+"/unit_importData.action";
			toMain(url);
		}
		
		function clearErrorData(){
			//alert(resultdata);
			var url =Global+"/unit_clearErrorData.action";
			var paremt={};
			paremt["sendStr"]=JSON.stringify(resultdata);
			$.post(url,paremt,function(data){
				var result=jQuery.parseJSON(data);
		    	...
		    	$("#tbodyunit tr").remove();
		    		//--------------
		    		var list = result.obj;
		    		$("#SXrow").tmpl(list).appendTo("#tbodyunit"); 
		    		//--------------
		    	}
			});
		}
