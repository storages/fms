...
<script type="text/javascript" src="${pageContext.request.contextPath}/js/public/public.js"></script>
 ...
 <script type="text/javascript">
  	...
  	function editData(id){
  		toMain("${pageContext.request.contextPath}/outstorage_editStorage.action?id="+ id +"&inOrOutFlag=out");
  	}
...
function gotoPage(pageNum, pageSize) {
	 var startDate = $("#beginDate").val();
		...
		var scmcocName = $("#scmcocName").val();
		var hsName = $("#hsName").val();
		var url = "${pageContext.request.contextPath}/outstorage_findAllOutStorage.action?startDate="+startDate+"&endDate="+endDate+"&scmcocName="+parse(scmcocName)+"&hsName="+parse(hsName)+"&currIndex=" + pageNum + "&maxIndex="+ pageSize ;
	// 在本窗口中显示指定URL的页面
	toMain(url);
}
  </script>
					...
					<table id="sample-table-1" class="table table-striped table-bordered table-hover"  style=" font-size: 12px;overflow: scroll;">
						<thead>
							<tr>
								...
								<th class="center" width="60px;">出库单号</th>
								...
								<th class="center" width="70px;">出库类型</th>
								<th class="center" width="60px;">物料类型</th>
								<th class="center" width="60px;">出库人</th>
								<th class="center" width="80px;">出库日期</th>
