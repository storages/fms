...
public class OutStorageAction extends BaseAction {
	...
	protected InStorage inStorage;// 出库
	...
	protected String expFlag;// 出库类型
	protected String outStorageNo;// 出库单号
	 ...
	 * @return
	 */
	public String findAllOutStorage() {
		try {
			...
			dataTotal = this.outStorageLogic.findDataCount("OutStorage", date, date2, parseValue(scmcocName), parseValue(hsName), impExpFlag);
			List storageList = this.outStorageLogic.findStorage(getLoginUser(), "OutStorage", date, date2, parseValue(scmcocName), parseValue(hsName), impExpFlag, (curr - 1) * DEFAULT_PAGESIZE,
					DEFAULT_PAGESIZE);
	...
	public String editStorage() {
		try {
			...
			Integer maxSerialNo = this.outStorageLogic.findMaxSerialNo(getLoginUser(), "OutStorage", impExpFlag);
			Integer serialNo = 0;
			...
			OutStorage storage = null;
			if (StringUtils.isBlank(id)) {
				storage = new OutStorage();
				storage.setSerialNo(serialNo);
			} else {
				storage = (OutStorage) this.outStorageLogic.findStorageById(OutStorage.class, id);
			}
			...
			this.request.put("outStorage", storage);
	...
	public void saveOutStorage() {
		try {
			...
			OutStorage outStorage = null;
			if (StringUtils.isNotBlank(id)) {
				outStorage = (OutStorage) this.outStorageLogic.findStorageById(OutStorage.class, id);
			} else {
			...
			outStorage.setSerialNo(serNo);// 流水号
			outStorage.setExpFlag(expFlag);// 出库类型
			outStorage.setImgExgFlag(imgExgFlag);// 物料标志
			outStorage.setPurchaseNo(purchaseNo);// 采购单号
			outStorage.setOutStorageNo(outStorageNo);// 出库单号
			outStorage.setOrderNo(orderNo);// 订单号
			Scmcoc scmcoc = this.scmcocLogic.findScmcocByName(this.parseValue(scmcocName));
			outStorage.setScmcoc(scmcoc);// 供应商
			Material material = this.materLogic.findMaterialByHsCode(getLoginUser(), hsCode);
			outStorage.setMaterial(material);// 物料清单
			...
			outStorage.setExpQty(qty);// 出库数量
			outStorage.setSpecQty(specQty);// 每件包装数量
			outStorage.setPkgs(pcs);// 件数
			Stock stock = null;
			if (StringUtils.isNotBlank(stockId)) {
				stock = this.stockLogic.findStockById(getLoginUser(), stockId);
			}
			outStorage.setStock(stock);// 仓库
			outStorage.setNote(this.parseValue(note));// 备注
			outStorage.setUseFlag("0");// 默认启用
			outStorage.setExpDate(new Date());// 出库日期
			outStorage.setHandling(this.getLoginUser().getLoginName());// 出库人
			String mess = this.outStorageLogic.saveStorage(this.getLoginUser(), outStorage);
	...
	public void importData() {
		...
		try {
			...
			title[8] = content[0][8];
			title[9] = content[0][9];
			if (!"物料标志".equals(title[0]) || !"出库类型".equals(title[1]) || !"出库单号".equals(title[2]) //
					|| !"订单号".equals(title[3]) || !"采购单号".equals(title[4]) || !"出库数量".equals(title[5])//
					|| !"物料编码".equals(title[6]) || !"仓库名称".equals(title[7]) || !"数量/(包装)".equals(title[8]) || !"备注".equals(title[9])) {
				result.setSuccess(false);
				result.setMsg("导入的excel文件内容不正确!");
			} else {
				List<TempOutStorage> tempOutStor = new ArrayList<TempOutStorage>();
				for (int i = 1; i < content.length; i++) {
					TempOutStorage outStorage = new TempOutStorage();
					outStorage.setImgExgFlag(content[i][0]);
					outStorage.setExpFlag(content[i][1]);
					outStorage.setOutStorageNo(content[i][2]);
					outStorage.setOrderNo(content[i][3]);
					outStorage.setPurchaseNo(content[i][4]);
					outStorage.setExpQty(content[i][5]);
					outStorage.setHsCode(content[i][6]);
					outStorage.setStockName(content[i][7]);
					outStorage.setSpecQty(content[i][8]);
					outStorage.setNote(content[i][9]);
					tempOutStor.add(outStorage);
				}
				// 开始验证
				List tlist = this.outStorageLogic.doValidata(tempOutStor);
				result.setSuccess(true);
				result.setObj(tlist);
			}
	...
	public String saveData() {
		...
		try {
			net.sf.json.JSONArray jsonArray = net.sf.json.JSONArray.fromObject(sendStr);
			List<TempOutStorage> list = net.sf.json.JSONArray.toList(jsonArray, new TempOutStorage(), new JsonConfig());
			if (null == list || list.size() <= 0) {
				out = response.getWriter();
				response.setContentType("application/text");
				response.setCharacterEncoding("UTF-8");
			...
			if (!this.outStorageLogic.doSaveExcelData(this.getLoginUser(), list)) {
				out = response.getWriter();
				response.setContentType("application/text");
				response.setCharacterEncoding("UTF-8");
				result.setSuccess(false);
