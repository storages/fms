...
public class QuotationAction extends BaseAction {
	...
	public String findQuotations() {
		try {
			...
			dataTotal = this.quotationLogic.findDataCount(getLoginUser(), parseValue(scmCocName), parseValue(hsCode), (begineffectDate == null || "".equals(begineffectDate)) ? null : date,
			...
			for (List<String> ldata : list) {
				...
				if (null != contents && contents.size() > 0) {
					try {
						...
						if ("".equals(msg)) {
							quotation.setPrice(contents.get(1) == null || "".equals(contents.get(1)) || "".equals(contents.get(1).trim()) || "null".equals(contents.get(1).trim()) ? null : Double
									.parseDouble(contents.get(1).trim()));// 单价
							quotation.setEffectDate(contents.get(2) == null || "".equals(contents.get(2)) || "".equals(contents.get(2).trim()) || "null".equals(contents.get(2).trim()) ? null
									: new SimpleDateFormat("yyyy-MM-dd").parse(contents.get(2).trim()));// 生效日期
							quotation.setNote((contents.get(3) == null || "".equals(contents.get(3)) || "".equals(contents.get(3).trim()) || "null".equals(contents.get(3).trim()) ? null
